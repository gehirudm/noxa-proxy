...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if this is a proxy purchase payment
    if (payment.type !== 'proxy_purchase') {
      console.log(`Payment ${orderId} is not a proxy purchase, forwarding to appropriate endpoint`);
      return NextResponse.json({ 
        success: false, 
        error: 'Not a proxy purchase payment',
        redirectTo: '/api/payments/callback/wallet'
      }, { status: 400 });
    }
    ...
    switch (paymentStatus) {
      case PaymentStatus.COMPLETED:
        ...
        await createTransactionRecord({
          ...
          description: payment.metadata?.description || `Proxy purchase ${orderId}`,
          metadata: {
      ...
      case PaymentStatus.REFUNDED:
        ...
        // If the proxy was already provisioned, we might need to revoke it
        if (payment.status === 'completed') {
          // Implement proxy revocation logic here
          console.log(`Payment ${orderId} was refunded after completion. Consider revoking proxy access.`);
          
          // Update the user's proxy plan information in Firestore to mark it as inactive
          const userRef = db.collection('users').doc(userId);
          if (payment.metadata?.proxyType) {
            await userRef.update({
              [`proxyPlans.${payment.metadata.proxyType}.isActive`]: false,
              [`proxyPlans.${payment.metadata.proxyType}.refundedAt`]: new Date(),
              updatedAt: new Date()
            });
          }
        }
    ...
    console.error('Error processing proxy purchase webhook:', error);
    return NextResponse.json(
